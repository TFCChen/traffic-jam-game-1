...
function App() {
  ...
  function enterEditor(level = current) {
    ...
    setMessage(savedCustom ? `正在編輯「${savedCustom.title}」。修改後請按「更新關卡」。` : "點選起點與終點，建立長度 2 或 3 的車輛。Target 必須位於第 3 列。");
  }
  ...
  function validateAndPlay() {
    ...
    const custom = { id: `preview-${Date.now()}`, difficulty: difficulty.label, title: existing?.title ?? "自製關卡預覽", cars: cloneCars(editorCars), analysis: difficulty };
    setCurrent(custom);
  ...
  function saveCustom() {
    ...
    const level = existing
      ...
      : { id: `custom-${Date.now()}`, title: `自製關卡 ${customLevels.length + 1}`, difficulty: difficulty.label, cars: cloneCars(editorCars), optimalMoves: solution.moves.length };
    const next = existing ? customLevels.map((item) => item.id === existing.id ? level : item) : [...customLevels, level];
    ...
    <div className="app-shell">
      ...
      <main className="workspace">
        <section className="game-column">
          ...
          <Board cars={mode === "editor" ? editorCars : cars} onMove={commitMove} highlightedCar={hint?.carId} editor={mode === "editor"} editorStart={editorStart} onCellClick={editorCell} onRemove={(id) => { setEditorCars((items) => items.filter((car) => car.id !== id)); setEditorStart(null); }} />
          {mode === "editor" && (
            <div className="editor-actions">
              {editorStart && <button onClick={cancelEditorStart}>取消選點</button>}
              <button onClick={() => { setEditorCars([]); setEditorStart(null); setMessage("已清空。請點選 Target 的起點。"); }}>清空</button>
        ...
        <aside className="side-panel">
          ...
          {panel === "custom" && (
            <div className="custom-list">
              {customLevels.length ? customLevels.map((level) => (
                <div className={`custom-item ${editingCustomId === level.id ? "editing" : ""}`} key={level.id}>
                  ...
                  <div className="custom-actions">
                    <button onClick={() => enterEditor(level)}>編輯</button>
                    <button className="danger" onClick={() => deleteCustom(level)}>刪除</button>
                  </div>
                </div>
              )) : <p>尚未儲存自製關卡。</p>}
            </div>
